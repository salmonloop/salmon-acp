...
5. 当前沉淀的通用经验规则：
   ...
   - 触发条件：新增或修改 ACP 方法、字段或能力声明；原生期望行为：ACP 标准方法必须严格对照官方 schema 与 capability 协商，自定义扩展只允许使用以下划线开头的方法名，且扩展 payload 字段只能由仓库内单一 contract 定义；禁止做法：把扩展 payload 冒充标准字段、继续兼容未声明的 legacy 扩展名、或在未 advertise capability 时实际执行 `fs`/`terminal`/交互请求；验证方式：标准方法覆盖 schema/行为测试，扩展方法覆盖 capability gating、method-not-found 和 contract round-trip 测试。详见 `docs/audit/acp-standard-vs-extension-contracts.md`。
   ...
   - 触发条件：缓存 Page、延迟加载区域或嵌套 ViewModel 暴露的 `x:Bind` 只读 UI 文本在 VM 属性已更新后仍显示旧值；原生期望行为：View 继续由 VM 的单一事实源驱动，并通过 XAML 生成绑定刷新机制投影到原生控件；禁止做法：在 code-behind 手动写 Button/TextBlock/AutomationProperties 等控件属性、在 ViewModel 反向感知控件状态、或新增第二套 UI 标题状态；验证方式：用合同测试断言属性变化只调用生成的 `Bindings.Update()` 且不手写控件属性，并用真实 GUI 路径覆盖 A-B-A 切换时选中态、正文和标题同源。
