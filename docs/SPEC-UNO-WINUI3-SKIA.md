...
- 运行脚本统一入口：`run.bat`
- MSIX runner：`.tools/run-winui3-msix.ps1`
  - 仅打包 MSIX：`build.bat msix`
...
**守门测试（必须保持通过）**
- `tests/SalmonEgg.Application.Tests/UiConventionsTests.cs`
...
`AccentBrush`（强调色）：
- 定义：`SalmonEgg/SalmonEgg/App.xaml`
- 使用：所有页面强调色必须引用 `{ThemeResource AccentBrush}`，不直接引用 `SystemAccentColor`。

**守门测试（必须保持通过）**
- `tests/SalmonEgg.Application.Tests/UiConventionsTests.cs` 会扫描 XAML 禁止旧键。
