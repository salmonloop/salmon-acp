...
## 8.1 会话/导航/搜索并发语义（硬性）
1. 会话激活必须走单一 owner（`INavigationCoordinator -> IConversationSessionSwitcher`），禁止多入口双写状态。
2. 必须遵守 latest-intent：旧请求失败/取消不得回滚最新用户选择，除非目标会话已无效。
3. 导航 UI 层只能表达意图，不得自行维护业务真相；真实状态仅来自 SSOT。
4. 全局搜索必须是显式状态机（`Idle/Loading/Results/Empty/Error`），并实现 latest-wins 防 stale 覆盖。
5. 搜索和会话切换主链路禁止同步阻塞（`.Result/.Wait`）。
6. ACP 协议调用必须按能力门控执行；未声明能力的方法不得调用。
